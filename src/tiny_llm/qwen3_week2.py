...
class Qwen3TransformerBlock:
    ...
    def __call__(
        ...
        self.offset = offset
        r = self.self_attn(self.input_layernorm(x), offsets=offset, cache=cache, mask=mask)
        h = x + r
...
class Qwen3ModelWeek2:
    ...
    def __call__(
        ...
        if len(cache) != self.num_hidden_layers:
            raise ValueError(
                f"Expected {self.num_hidden_layers} cache objects, "
                f"got {len(cache)}"
            )
        if isinstance(offset, int):
